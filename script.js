/* ==========================================================================
   SmallBizBoost RI — script.js (FULL MERGED VERSION)
   ========================================================================== */
...
const CENSUS_API_KEY = "47d157768272a26f195d448b5913630c15f1531f";
const ACS_YEAR = 2022;
...
// Faster, stable JSON fetch using AllOrigins /get endpoint
async function fetchJson(url, attempt = 1) {
  try {
    // Use the faster AllOrigins endpoint
    const proxied = `https://api.allorigins.win/get?url=${encodeURIComponent(url)}`;

    const res = await fetch(proxied);
    const data = await res.json();

    // AllOrigins wraps the real response inside data.contents
...
async function fetchUnemploymentRate(countyFips, countyId) {
  ...
  if (!series || !series.length) {
    return FALLBACK_ECONOMIC_DATA[countyId].unemployment;
  }
...
/* ------------------------------------------------------------------------
   fetchAllEconomicData — FULL VERSION
   ------------------------------------------------------------------------ */
...
async function fetchAllEconomicData() {
  ...
  countyIds.forEach((id) => {
    ...
    sources[id] = {
      income: "fallback",
      priorIncome: "fallback",
      growth: "fallback",
      population: "fallback",
      establishments: "fallback",
      selfEmployed: "fallback",
      wage: "fallback",
  ...
  const results = await Promise.allSettled([
    ...
    ...countyIds.map((id) =>
      fetchUnemploymentRate(COUNTY_FIPS[id], id)
    ),
  ]);
  ...
  if (incomePopRes.status === "fulfilled") {
    countyIds.forEach((id) => {
      ...
      if (Number.isFinite(income)) {
        records[id].income = income;
        sources[id].income = "live";
      }
      if (Number.isFinite(population)) {
        records[id].population = population;
        sources[id].population = "live";
      }
    });
  }
  ...
  if (priorIncomeRes.status === "fulfilled") {
    countyIds.forEach((id) => {
      ...
      if (Number.isFinite(v)) {
        records[id].priorIncome = v;
        sources[id].priorIncome = "live";
      }
    });
  }
  ...
  if (selfEmpRes.status === "fulfilled") {
    countyIds.forEach((id) => {
      ...
      if (Number.isFinite(v)) {
        records[id].selfEmployed = v;
        sources[id].selfEmployed = "live";
      }
    });
  }
  ...
  if (earningsRes.status === "fulfilled") {
    countyIds.forEach((id) => {
      ...
      if (Number.isFinite(v)) {
        records[id].wage = v / 52;
        sources[id].wage = "live";
      }
    });
  }
  ...
  if (estabRes.status === "fulfilled") {
    countyIds.forEach((id) => {
      ...
      if (Number.isFinite(v)) {
        records[id].establishments = v;
        sources[id].establishments = "live";
      }
    });
  }
